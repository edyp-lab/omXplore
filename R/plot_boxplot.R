...
boxPlot <- function(dataIn,
  ...
  if (is.null(pal)) {
    ...
    myColors <- GetColorsForConditions(
      conds,
  ...
  add_variable_to_series_list <- function(
      x,
      series_list,
      key_vector,
      value_vector) {
  ...
  ## Boxplot function:
  make_highchart_boxplot_with_colored_factors <- function(
      value,
      by,
    ...
    series2 <- lapply(seq_along(series), function(x) {
      add_variable_to_series_list(
        x = x, series_list = series,
