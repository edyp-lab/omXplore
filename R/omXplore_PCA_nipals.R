...
#'
#' @param X a data.frame() of quantitative data
#' @param scale.unit See `FactoMineR::PCA()`
#' @param ncp See `FactoMineR::PCA()`
#' @param ind.sup See `FactoMineR::PCA()`
#' @param quanti.sup See `FactoMineR::PCA()`
#' @param quali.sup See `FactoMineR::PCA()`
#' @param row.w See `FactoMineR::PCA()`
...
my_PCA <- function(
  ...
  if (!is.null(ind.sup)) {
    ...
    res.ind.sup <- list(
      coord = coord.ind.sup, cos2 = cos2.ind.sup,
      dist = sqrt(dist2)
    )
  ...
  if (!is.null(quali.sup)) {
    ...
    for (i in 1:ncol(X.quali.sup)) {
      ...
      if ((levels(var)[1] %in% (1:nrow(X))) | (levels(var)[1] %in%
        ...
        row.names(bary) <- as.character(levels(var))
      }
      if (i == 1) {
        barycentre <- bary
      } else {
        barycentre <- rbind(barycentre, bary)
      }
    }
    ...
    res.quali.sup <- list(
      coord = coord.barycentre, cos2 = cos2.bary.sup,
      v.test = vtest, dist = sqrt(dist2), eta2 = eta2
    )
    call.quali.sup <- list(
      quali.sup = X.quali.sup, modalite = modalite,
